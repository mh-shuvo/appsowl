	...
	<div class="col-sm-12">
		<div class="ibox">
			<div class="ibox-title">
				<h3> <?php echo trans('income_report'); ?> </h3>
			</div>
		</div>
	</div>
</div>
	...
	<div class="col-sm-12">
		<div class="ibox">
			...
			<div class="ibox-content table-responsive">
				<table class="table table-striped table-bordered" id="expenseReportTable" data-title="Expense Report"></table>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	function TableDataColums(){
		return [
		{ "title": $_lang.income_date,"class": "text-center", data : 'date' },
		{ "title": $_lang.income_purpose,"class": "text-center", data : 'cr_account' },
		{ "title": $_lang.prayer_name,"class": "text-center", data : 'payer_name' },
		{ "title": $_lang.income_status,"class": "text-center", data : 'account_status' },
		{ "title": $_lang.payment_method,"class": "text-center", data : 'payment_method' },
		{ "title": $_lang.income_amount,"class": "text-center account_amount", data : 'amount' }, 
		{ "title": $_lang.added_by,"class": "text-center", data : 'amount' },
		{ "title": $_lang.created_at,"class": "text-center", data : 'created_at' },
		{ "title": $_lang.action,"class": "text-center not-show",
			orderable: false,
			render: function(data, type, row){
				var html ='<button class="btn btn-sm btn-danger AccountDelete" data-account="'+row.id+'"><i class="fa fa-trash"></i></button>';
				return html;
			}
		}
		];
	...
	function ExpenseFilter(FilterType) {
		...
		var expenseFor= $('#expense_for').val() || 0;
		...
		AS.Http.GetDataTable('#expenseReportTable',TableDataColums(),{ action : "GetIncomeReportData",account_name : ac_name, between_action : "date", from_data : start, to_data : end, date_range : true, account_for : expenseFor, store_id : businessLocation},"pos/filter/",FilterType,false);
	}
