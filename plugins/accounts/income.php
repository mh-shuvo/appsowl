		...
		<div class="col-lg-12">
			<div class="ibox float-e-margins">
				...
				<div class="ibox-title">
					<h2><?php echo trans('income_list'); ?></h2>
					<?php if(app('pos')->checkPermission('accounts','edit',true)){ ?>
						<a href="javascript:void(0)" class="btn btn-success add_account pull-right add_variation_btn"><i class="fa fa-plus-circle"></i> <?php echo trans('add'); ?></a>
					<?php } ?>
				</div>
				...
				<div class="ibox-content">
							...
							<?php
								...
								<div class="col-sm-3">
									<div class="form-group">
										...
										<select class="form-control get_income_list_filter" id="store_id" name="store_id">
							...
							<div class="col-sm-3">
								<div class="form-group" id="date_1">
									...
									<div class="input-group">
										...
										<input class="form-control get_income_list_filter" type="text" name="daterange" id="reportrange" placeholder="<?php echo trans('custom_daterange'); ?>" readonly />
									</div>
								</div>
							</div>
							<div class="col-sm-3">
								<label class="control-label"><?php echo trans('income_for'); ?>: </label>
								<div class="input-group" wfd-id="17">
									<span class="input-group-addon" wfd-id="19"><i class="fa fa-user"></i></span>
									<select class="form-control get_income_list_filter" id="income_for">
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover income_table" data-title="Income List"></table>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(document).ready(function(){
		...
		function CustomDateRangeFilter(FilterType) {
			...
			var incomeFor= $('#income_for').val() || 0;
			...
			AS.Http.GetDataTable('.income_table',TableDataColums(),{ action : "GetIncomeData",from_data : start, to_data : end, account_for : incomeFor, store_id : businessLocation},"pos/filter/",FilterType,['account_amount'],0,100,{'amount':"0.00"});
		}
		...
		$(document).on("click",".ViewAccount", function(){
			...
			location.href="pos/income-report/"+name;
		});
		
		$(document).on("change",".get_income_list_filter", function(){
			CustomDateRangeFilter(true);
		});
		...
		$(document).on("click",".add_account", function(){
			var add_account_type = 'account_income';
			$(".show_modal").remove();
