<?php defined('_AZ') or die('Restricted access');
	...
	<script>
		...
		function TableDataColums(){
			return [
			{ "title": $_lang.payment_method_id,"class": "text-center", data : 'payment_method_id' },
			{ "title": $_lang.payment_method_name,"class": "text-center", data : 'payment_method_name' },
			{ "title": $_lang.account_type,"class": "text-center", data : 'payment_method_type' },
			...
			{ "title": $_lang.action,"class": "text-center",
				orderable: false,
				render: function (data, type, row) {
					let del = '<li><a href="javascript:void(0)" name="'+row.name+'" payment_method_id="'+row.payment_method_id+'"   class="delete_payment_method"><i class="glyphicon glyphicon-trash"></i> '+$_lang.delete+'</a></li>';
					
					if(row.payment_method_type == 'account_cash'){
						del = '';
					}
