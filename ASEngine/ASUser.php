	...
	*/
	class ASUser
	{
		...
		*/
		...
		public function withdrawalPassword($userId, array $data)
		{
			if ($data['new_password'] !== $data['new_password_confirmation']) {
			}
			
			$this->updateInfo($userId, array(
            "withdrawal_password" => $this->hashPassword($data['new_password'])
			));
		...
		public function updateWithdrawalPassword($userId, array $data)
		{
			if ($errors = $this->validatePasswordUpdate($userId, $data)) {
				respond(array('errors' => $errors), 422);
			}
		...
		public function updateAdminPassword($userId, array $data)
		{
			if ($errors = $this->validatePasswordUpdate($userId, $data)) {
				respond(array('errors' => $errors), 422);
			}
		...
		public function updateAdminUserPassword($userId, array $data){
			
			if ($errors = $this->validateUserPasswordUpdate($userId, $data)) {
				respond(array('errors' => $errors), 422);
			}
		...
		*/
		...
		private function validateUserPasswordUpdate($userId, array $data)
		{
			$errors = array();
